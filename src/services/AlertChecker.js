...
export async function computeMetric(metric, domain, userId, frequency) {
  const now = new Date();
...
export function shouldFire(measured, condition, threshold) {
  if (condition === "gt") return measured > threshold;
  if (condition === "lt") return measured < threshold;
  return false;
}

export function isDueForCheck(lastCheckedAt, frequency) {
...
export async function runAlertChecks(userId) {
  ...
  for (const alert of enabledAlerts) {
    ...
    const measuredValue = await computeMetric(
      alert.metric,
      alert.domain,
      alert.user_id,
      alert.frequency,

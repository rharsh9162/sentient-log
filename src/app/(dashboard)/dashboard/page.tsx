...
import { Activity, Timer, AlertCircle, Globe, Loader2, Filter } from 'lucide-react';
...
export default function OverviewPage() {
  ...
  const kpis = [
    { label: 'Total Events', value: stats?.total_events?.toLocaleString() ?? '—', icon: Activity, color: 'indigo' },
    { label: 'Avg Latency', value: stats ? `${stats.avg_latency}ms` : '—', icon: Timer, color: 'green' },
    { label: 'Errors', value: stats?.error_count?.toLocaleString() ?? '—', icon: AlertCircle, color: 'red' },
    { label: 'Unique URLs', value: stats?.unique_urls?.toLocaleString() ?? '—', icon: Globe, color: 'amber' },
  ];
      ...
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p className="page-subtitle">
            {domain ? `Showing data for ${domain}` : 'Your platform at a glance'}
          </p>
        </div>
        ...
        {kpis.map(({ label, value, icon: Icon, color }) => (
          <div className="kpi-card" key={label}>
            ...
            <div>
              ...
              {loading ? <div className="kpi-skeleton" /> : <p className="kpi-value">{value}</p>}
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="card">
        ...
        {loading ? (
          ...
          <p style={{ color: '#64748B', fontSize: 14, textAlign: 'center', padding: 40 }}>
            No event data yet. Go to <strong>Analyze Site</strong> to crawl a real website.
          </p>
        )}
      </div>
          ...
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={stats.top_urls} layout="vertical">
                ...
                <YAxis type="category" dataKey="url" tick={{ fill: '#475569', fontSize: 10 }} axisLine={false} tickLine={false} width={130} />
                <Tooltip
                  contentStyle={{ background: 'rgba(255,255,255,0.9)', border: '1px solid #E2E8F0', borderRadius: 8, fontSize: 13, backdropFilter: 'blur(10px)' }}
                  labelStyle={{ color: '#475569' }}
                  itemStyle={{ color: '#7C3AED' }}
            ...
            <p style={{ color: '#64748B', fontSize: 14, textAlign: 'center', padding: 40 }}>No data</p>
          )}
        </div>
          ...
          {loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {stats.event_type_breakdown.map(({ type, count }) => {
                const max = Math.max(...stats.event_type_breakdown.map(e => e.count));
                const pct = (count / max) * 100;
                const colors: Record<string, string> = {
                  page_view: '#6366F1', click: '#22C55E', error: '#EF4444', api_call: '#F59E0B',
                };
                return (
                  <div key={type}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                      <span className={`badge badge-${type}`}>{type.replace('_', ' ')}</span>
                      <span style={{ fontSize: 13, color: '#475569', fontWeight: 500 }}>{count}</span>
                    </div>

...
export default function AskPage() {
  ...
  const ask = async (question: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'ai', content: `⚠️ ${message}. Make sure your Gemini API key is configured in .env.local` },
      ]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p className="page-subtitle">
            {domain ? `Asking about ${domain}` : 'Ask questions about your logs in plain English'}
          </p>
        </div>
        {domains.length > 0 && (
      ...
      <div className="chat-container">
        ...
        <div className="chat-input-area">
          {domain && (
            <div style={{ padding: '6px 16px', fontSize: 11, color: '#7C3AED', background: 'rgba(139,92,246,0.08)', borderBottom: '1px solid rgba(139,92,246,0.15)' }}>
          ...
          <div className="chat-input-row">
            <input
              ...
              placeholder={domain ? `Ask about ${domain}...` : 'Ask a question about your logs...'}
              disabled={loading}
            />
            <button
